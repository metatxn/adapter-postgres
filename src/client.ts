  ...
  implements IDatabaseCacheAdapter
{
  ...
  // Modify the searchMemoriesByEmbedding method in client.ts
  async searchMemoriesByEmbedding(
    ...
    return this.withDatabase(async () => {
      ...
      // Early return with empty array if embedding is empty
      if (!embedding || embedding.length === 0) {
        elizaLogger.warn(
          "Empty embedding vector provided, returning empty result"
        );
        return [];
      }

      // Validate embedding dimension
      const expectedDimensions = getEmbeddingConfig().dimensions;
      if (embedding.length !== expectedDimensions) {
        // Instead of throwing, try to handle the issue
        elizaLogger.warn(
          `Invalid embedding dimension: expected ${expectedDimensions}, got ${embedding.length}. Will attempt to fix.`
        );

        // If empty or invalid, return empty result set rather than error
        ...
        if (embedding.length < expectedDimensions) {
          ...
          while (paddedEmbedding.length < expectedDimensions) {
            paddedEmbedding.push(0);
          }
          embedding = paddedEmbedding;
          elizaLogger.info(
            `Padded embedding vector to ${expectedDimensions} dimensions`
          );
        }
        ...
        if (embedding.length > expectedDimensions) {
          ...
          elizaLogger.info(
            `Truncated embedding vector to ${expectedDimensions} dimensions`
          );
        }
      }
